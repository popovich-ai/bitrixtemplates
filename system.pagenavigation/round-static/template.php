...
// убираем из текущего url-адреса уже имеющуюся нумерацию
$arResult['sUrlPath'] = preg_replace('|\/page(\d+)\/|','/',$arResult['sUrlPath']);
$arResult['sUrlPathParams'] = preg_replace('|\/page(\d+)\/|','/',$arResult['sUrlPathParams']);
...
<?// TODO: когда понадобится пагинация в обратном порядке - внести правки тут?>
...
<?else:?>
	...
	<?if ($arResult["NavPageNomer"] > 1):?>
		...
		<?else:?>
			<?if ($arResult["NavPageNomer"] > 2):?>
                <?
                    $pageURL = $arResult['sUrlPath'].'page'.($arResult['NavPageNomer']-1).'/';
                    if(strlen($strNavQueryString)>0){
                        $pageURL = $pageURL.'?'.$strNavQueryString;
                    }
                ?>
				<li class="bx-pag-prev"><a href="<?=$pageURL?>"><span><?echo GetMessage("round_nav_back")?></span></a></li>
			<?else:?>
				<li class="bx-pag-prev"><a href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>"><span><?echo GetMessage("round_nav_back")?></span></a></li>
			<?endif?>
			<li class=""><a href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>"><span>1</span></a></li>
	...
	?>
		...
		<?else:?>
            <?
            $pageURL = $arResult['sUrlPath'].'page'.($arResult['nStartPage']).'/';
            if(strlen($strNavQueryString)>0){
                $pageURL = $pageURL.'?'.$strNavQueryString;
            }
            ?>
			<li class=""><a href="<?=$pageURL;?>"><span><?=$arResult["nStartPage"]?></span></a></li>
		<?endif?>
		<?$arResult["nStartPage"]++?>
	<?endwhile?>
	...
	<?if($arResult["NavPageNomer"] < $arResult["NavPageCount"]):?>
		<?if($arResult["NavPageCount"] > 1):?>
			<?
			$pageURL = $arResult['sUrlPath'].'page'.($arResult['NavPageCount']).'/';
			if(strlen($strNavQueryString)>0){
				$pageURL = $pageURL.'?'.$strNavQueryString;
			}
			?>
			<li class=""><a href="<?=$pageURL;?>"><span><?=$arResult["NavPageCount"]?></span></a></li>
		<?endif?>
            <?
            $pageURL = $arResult['sUrlPath'].'page'.($arResult['NavPageNomer']+1).'/';
            if(strlen($strNavQueryString)>0){
            ...
            ?>
			<li class="bx-pag-next"><a href="<?=$pageURL?>"><span><?echo GetMessage("round_nav_forward")?></span></a></li>
	<?else:?>
		<?if($arResult["NavPageCount"] > 1):?>
			<li class="bx-active"><span><?=$arResult["NavPageCount"]?></span></li>
		<?endif?>
